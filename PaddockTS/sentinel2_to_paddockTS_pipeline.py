...
def _run_pipeline_steps(query, statuses, times, log, update_callback=None):
    import xarray as xr
        ...
        try:
            with redirect_stdout(log), redirect_stderr(log):
                if i == 0:
                    ...
                    if exists(query.sam_paddocks_path):
                    ...
                    sentinel2_video_with_paddocks(query, query.sam_paddocks_path, ds_sentinel2=ds_sentinel2)
                    ...
                    fractional_cover_paddocks_video(query, query.sam_paddocks_path, ds_fractional_cover=ds_fractional_cover, ds_sentinel2=ds_sentinel2)
...
def run(query: Query, reload: bool = False):
    ...
    with Live(Group(_make_table(statuses, times), progress), console=_console, refresh_per_second=4) as live:
        ...
        _run_pipeline_steps(query, statuses, times, log, update_callback=update_display)
