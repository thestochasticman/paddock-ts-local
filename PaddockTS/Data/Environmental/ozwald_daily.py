"""
Download daily climate variables from OzWald.

Catalog: https://thredds.nci.org.au/thredds/catalog/ub8/au/OzWALD/daily/meteo/catalog.html
"""
import os
import json
...
def _singleyear_thredds(var, latitude, longitude, buffer, year, stub, tmp_dir, verbose=True):
...
def _singleyear_gdata(var, latitude, longitude, buffer, year):
    prefix = ".daily" if var == "Pg" else ""
    filename = f"/g/data/ub8/au/OzWALD/daily/meteo/{var}/OzWALD{prefix}.{var}.{year}.nc"
...
def ozwald_daily(
    ...
    ----------
        ...
        variables: List of variables to download (default: ["Pg", "Tmax", "Tmin", "Uavg", "Ueff", "VPeff"])
    ...
    if variables is None:
        variables = ["Pg", "Tmax", "Tmin", "Uavg", "Ueff", "VPeff"]
