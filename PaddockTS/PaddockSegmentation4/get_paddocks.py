"""
High-level orchestrator for PaddockSegmentation4 pipeline.
...
3. Watershed segmentation with distance transform markers

Key improvement over PaddockSegmentation3:
- Uses distance transform to find paddock centers naturally
- Better for irregular shapes than percentile thresholding
"""

from os.path import exists

from PaddockTS.PaddockSegmentation4._1_presegment import presegment
from PaddockTS.PaddockSegmentation4._2_segment import segment
...
def get_paddocks(
    ...
    Uses time series K-means + cv2.findContours for polygon extraction.
    ...
    Args:
        ...
        k_range: Range of k values to try when n_clusters='auto' (default 3-15)
    ...
    # Stage 2: Extract polygons using cv2.findContours
    segment(
        query,
        min_area_ha=min_area_ha,
        max_area_ha=max_area_ha,
